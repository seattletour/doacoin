...
#if defined(NDEBUG)
# error "Doacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8ceeba4a35a756d94c2262ea1f0edc858778224455c15363d3537f460e2ab795");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Doacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Doacoin Signed Message:\n";
...
{
    // Doacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy;
    if(nHeight == 1)
        nSubsidy = 463798686 * COIN;
    else
        nSubsidy = 1 * COIN;

    // Subsidy is cut in half every 262800 blocks, which will occur approximately every 1 years
    nSubsidy >>= (nHeight / 262800); // Doacoin: 262800 blocks in ~1 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 60; // Doacoin: 2 minutes
static const int64 nTargetSpacing = 2 * 60; // Doacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Doacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Doacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7304759f938e028f712544dfb4515ba26f6751133ee7ada6be8f57e2b5803fc4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "DOA is Decentralization On Asset";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04301fa6d05209d4a584def8c7bcd083015fbc9ac0cc215d632d64be6f81de6c609a1726b1d9dda3bdc74200c8cd29ec16d2680d38b1d6d5c3515416d77239114f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1606032196;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087315779;

        if (fTestNet)
        {
            block.nTime    = 1606032185;
            block.nNonce   = 387674421;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x0064526c118099820191a461c783e1a7ccf37c36a3292bc5fffef5de2dfd1c83"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xaf, 0xbe, 0xcd, 0xdc }; // Doacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DoacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DoacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DoacoinMiner(CWallet *pwallet)
{
    printf("DoacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("doacoin-miner");
    ...
    try { loop {
        ...
        printf("Running DoacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DoacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DoacoinMiner, pwallet));
}
